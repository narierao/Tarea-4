    ...
    while actual_time < max_time:
        ...
        if next_event == next_client_at:
            ...
            if len(client_looking_for) + len(queue_client_list)+ n < 30:
                ...
                if client1_in_casher == None and client2_in_casher == None and client3_in_casher == None and len(queue_client_list) == 0 and len(client_looking_for) == 0:
        ...
        # first casher process a product
            ...
            if client1_in_casher.quantity_products == 0:
                ...
                if queue_client_list != []:
                    ...
                    client1_next_product = actual_time + ExponentialInstance(1/5)
        ...
        # second casher process a product    
            ...
            if client2_in_casher.quantity_products == 0:
                ...
                if queue_client_list != []:
                    ...
                    client2_next_product = actual_time + ExponentialInstance(1/5)
                else:
                    client2_next_product = max_time +1
                    client2_in_casher = None
            else:
            ...
            if client3_in_casher.quantity_products == 0:
                ...
                if queue_client_list != []:
                    ...
                    if actual_time >= 4*60*60 and actual_time <= 10*60*60:
                        ...
                        cashier3.change_params(4, 16, 0)
                    client3_next_product = actual_time + ExponentialInstance(1/5)
                else:
                    client3_next_product = max_time +1
                    client3_in_casher = None
            else:
...
def simlations():
    ...
    print("Clientes atendidos:",np.mean(attended_clients))
    print("Tiempo en cola:", np.mean(queue_means)/60)
    print("Largo cola promedio:", np.mean(mean_queue_longs))
    print("Tienda vacía:", np.mean(emptys)/60)
    print("Tres cajeros ocupados:", np.mean(fully_cashers))
    print("Clientes perdidos:", np.mean(lost_clients), "\n\n")
